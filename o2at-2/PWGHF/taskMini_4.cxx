...
{
  return candidate.y(hfHelper.mass(pdg::kD0));
}
} // namespace hf_cand_prong2
...
struct HfCandidateSelectorD0 {
  ...
  bool selectionTopolConjugate(const T1& candidate, const T2& trackPion, const T2& trackKaon)
  {
    ...
    // invariant-mass cut
    if (trackPion.sign() > 0) {
      if (std::abs(invMassD0(candidate) - hfHelper.mass(pdg::kD0)) > cuts->get(binPt, "m")) {
        return false;
      }
    } else {
      if (std::abs(invMassD0bar(candidate) - hfHelper.mass(pdg::kD0)) > cuts->get(binPt, "m")) {
        return false;
      }
    }
    return true;
...
struct HfTaskD0 {
  ...
  void process(aod::Collision const&,
               ...
               soa::Filtered<soa::Join<aod::HfCandProng2, aod::HfSelCandidateD0>> const& candidates)
  {
